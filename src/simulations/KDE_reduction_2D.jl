...
Function that performs kernel density estimation (KDE) on 2 dimensions.
...
Returns:
    The discretised matrix.
    TODO: discuss whether it is better to return the function KDE or the discretized matrix
        (in the format we use at the level of the code)
...
function KDE_reduction_2D(end_state, boundaries, nb_bin, dim1,dim2)
    ...
    # defines the space over which the function KDE is evaluated
    xspan=Span_2D(boundaries,nb_bin,dim1,dim2)

    # creates the discretized matrix of KDE over the space previously defined
    for i = 1:nb_bin
        for j = 1:nb_bin
    ...
    # returns the discretised matrix
    return U_kde
